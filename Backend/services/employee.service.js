...
async function getHourAceptedBy(id, fechaInicio, fechaFin, res) {
	try {
		...
		if (!fechaInicio && !fechaFin) {
			...
			const fechaInicioPre = moment(fechaInicio)
				.add(-1, 'days')
				.format('YYYY-MM-DD 00:00:00');
			const fechaFinPre = moment(fechaFin).format('YYYY-MM-DD 23:59:59');

			queryHours = {
				where: {
					employeeId: id,
					fechaEvento: { [Op.between]: [fechaInicioPre, fechaFinPre] },
				},
			};
		}
...
async function getTimeRequestByEmployeeId(id, res) {
	try {
		...
		const TimeRequests = await models.TimeRequest.findAll({
			where: { employeeId: id },
			include: [
				{
					model: models.Employee,
					as: 'employee',
					include: [{ model: models.User, as: 'User', attributes: atributeUser }],
					attributes: ['supervisor', 'lugarDeTrabajo'],
				},
			],
		});
...
async function getRequest(req, res) {
	try {
		...
		if (Requests.length === 0)
			return res
				.status(403)
				.json({ message: 'El empleado no tiene solicitudes registradas' });
...
async function getTimeRequest(req, res) {
	try {
		...
		if (req.user.rollTypeId === 1) {
			...
			const TimeRequest = await models.TimeRequest.findAll({
				where: { employeeIdRequest: req.user.id },
				include: [
					{
						model: models.Employee,
						as: 'employee',
...
async function createRequest(params, id, next) {
	try {
		...
		const request = await models.Request.create({
			...
			statusId: params.statusId,
			descripcionEmpleado: params.descripcionEmpleado,
			descriptionRespuesta: params.descriptionRespuesta,
...
async function registerEvents(params, id) {
	try {
		...
		return res
			.status(404)
			.json({ message: 'Ya se tiene registro de la hora de entrada', ok: false });
	}
}

async function sendInformationByAccessCode(params) {
	const employee = await models.Employee.findOne({
		where: { accessCode: params.headers['accesscode'] },
	});
	...
	const token = jwt.sign({ sub: employee.userId }, config.secret, {
		expiresIn: '2h',
	});
			...
			switch (roll) {
				case 1:
					...
					if (typeof name === 'undefined') {
						params = {
							include: [{ model: models.User, as: 'User', attributes: atribute }],
							attributes: atributeEmployee,
						};
					}
					break;
		...
		switch (eventActionTypeId) {
			case '1':
				for (const employeeSchedul of employeeSchedules) {
					const schedule = await models.Schedule.findByPk(
						employeeSchedul.scheduleId
					);
					...
					switch (day) {
						case '0':
							if (schedule.Domingo) {
								...
								const diferencia = moment(fechaHoraCreacion).diff(
									fechaEntrada,
									'seconds'
								);
			...
			case '3':
				for (const employeeSchedul of employeeSchedules) {
					const fechaInicioPre = moment()
						.tz(process.env.TZ)
						.format('YYYY-MM-DD 00:00:00');
					const fechaFinPre = moment()
						.tz(process.env.TZ)
						.format('YYYY-MM-DD 23:59:59');
					const schedule = await models.Schedule.findByPk(
						employeeSchedul.scheduleId
					);
					const eventsPre = await models.Event.findAll({
						where: {
					...
					if (eventsPre.length === 0)
						res
							.status(202)
							.json({ message: 'Empleado no tiene registros de salida a comer' });

					let day = moment(fechaCreacion).format('d');

					//if(eventsPre.length > 2) throw 'Empleado tiene más de un registro de salida a comer';
					if (eventsPre.length > 2)
						return res
							.status(202)
							.json({
								message: 'Empleado tiene más de un registro de salida a comer',
							});
			...
			//Agregar fecha
			case '5':
				const employeeTimeRequests = await models.TimeRequest.findAll({
					where: {
						employeeId: id,
						statusId: 2,
				...
				if (employeeTimeRequests.length === 0)
					return res.status(202).json({ message: 'Empleado no tiene horas extra' });

				for (const employeeTimeRequest of employeeTimeRequests) {
					const fechaEntrada = employeeTimeRequest.fechaAsignacion.concat(
						' ',
						employeeTimeRequest.horaAsignacion
					);
			...
			case '6':
				...
				if (eventsPre.length === 0)
					return res
						.status(202)
						.json({ message: 'Empleado no tiene registros de entrada a hora extra' });

				const employeeTimeRequests1 = await models.TimeRequest.findAll({
					where: {
						employeeId: id,
						statusId: 2,
						fechaAsignacion: fechaCreacion,
				...
				if (employeeTimeRequests1.length === 0)
					return res.status(202).json({ message: 'Empleado no tiene horas extra' });
				break;
...
async function validacionNumeroAleatorio() {
	...
	do {
		...
		const accessCode = await models.Employee.findOne({
			where: { accessCode: num },
		});
...
async function getEmployeeScheduleById(id, res) {
	try {
		...
		if (employee.schedule.length == 0)
			throw new Error('Empleado sin registro de horario');
...
async function deleteEmployeeScheduleById(id, res) {
	try {
		const employeeSchedule = await models.EmployeeSchedule.findOne({
			where: { id: id },
		});
...
function checkAccessCode() {
	return [
		async (req, res, next) => {
			const employee = await models.Employee.findOne({
				where: { accessCode: req.headers['accesscode'] },
			});

			if (!employee)
				return res
					.status(404)
					.json({ message: 'Código de acceso no encontrado', ok: false });
...
async function registerCheck(params) {
	try {
		...
		await models.Check.create({
			employeeId: employee.id,
			DateCheck: fechaCheck,
			longitudeCheck: params.longitudeCheck,
			latitudeCheck: params.latitudeCheck,
			tipoCheck: params.tipoCheck,
		});
	} catch (error) {
		return res
			.status(404)
...
async function reviewOut(employee) {
	...
	registroEntradaEmpleado = await models.Check.findOne({
		where: {
			employeeid: employee.id,
			dateCheckIn: { [Op.between]: [fechaInicio, fechaFin] },
		},
	});
	...
	registroSalidaEmpleado = await models.Check.findOne({
		where: {
			employeeid: employee.id,
			dateCheckOut: { [Op.between]: [fechaInicio, fechaFin] },
		},
	});
