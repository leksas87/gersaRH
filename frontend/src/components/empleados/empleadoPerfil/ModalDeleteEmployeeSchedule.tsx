...
const ModalDeleteEmployeeSchedule = () => {
	...
	const hanleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
		e.preventDefault();
		dispatch(deleteEmployeeSchedules(schedulesToDelete.id));
	};
	...
	return (
		<div>
			...
			<div
				...
				id='ModalDeleteEmployeeSchedule'
				tabIndex={-1}
				aria-labelledby='ModalDeleteEmployeeSchedule'
				aria-hidden='true'
			>
						...
						>
							...
							>
								...
								>
									...
									>
										Se eliminará la asignación del horario{' '}
										<span className='fw-bold'>{schedulesToDelete.scheduleName} </span> de{' '}
										<span className='fw-bold'>{perfilUsuario.firstName}</span>{' '}
										<span className='fw-bold'>{perfilUsuario.lastName}</span>
									</label>
								</div>
...
export default ModalDeleteEmployeeSchedule;
